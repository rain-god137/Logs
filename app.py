...
def index():
    ...
    total_task = db.session.execute(
        select(func.count(Tasks.taskName)).where(
            Tasks.username == current_user.username
        )
    ).scalar()
    total_completed_task = db.session.execute(
        select(func.count(Tasks.taskName)).where(
            Tasks.username == current_user.username, Tasks.completed == 1
        )
    ).scalar()
    completion_rate = (
        round(total_completed_task / total_task * 100, 2) if total_task else 0
    )
    xp_calc()
    level_up()
    calculate_streak()
    ...
    user_stat = db.session.execute(
        select(Level_ups).filter_by(username=username)
    ).scalar_one_or_none()
    ...
    xp_next_level = math.ceil(
        120 * ((user_level) ** 1.5)
    )  # xp required to cross the current level and reach the next level.
    ...
    return render_template(
        ...
        taskCompleted=total_completed_task,
...
def level_up():
    stat = db.session.execute(
        select(Level_ups).where(Level_ups.username == current_user.username)
    ).scalar_one_or_none()
    ...
    for i in range(1, len(xp_req)):
        if xp_req[i - 1][1] <= c_xp < xp_req[i][1]:
...
def xp_required(level):
    ...
    xp_required_window = [
        (j, math.ceil(120 * (j**1.5))) for j in window
    ]  # each index is a tuple (level, xp required to cross that level)
...
def calculate_streak():
    completed_dates = (
        db.session.execute(
            select(Tasks.date)
            .filter_by(username=current_user.username, completed=1)
            .distinct()
            .order_by(desc(Tasks.date))
        )
        .scalars()
        .all()
    )
    stat_user = db.session.execute(
        select(Level_ups).where(Level_ups.username == current_user.username)
    ).scalar_one_or_none()
...
def xp_calc():
    xp_stat = db.session.execute(
        select(Level_ups).filter_by(username=current_user.username)
    ).scalar_one_or_none()
...
def heatmap():
    day_magnitude = db.session.execute(
        select(Tasks.date, func.count(Tasks.taskName))
        .where(Tasks.username == current_user.username, Tasks.completed == 1)
        .group_by(Tasks.date)
    )
    ...
    for i in reversed(range(time_delta)):
        date_to_check = (datetime.now(tz=timezone.utc).date() - timedelta(days=i)).strftime(
            "%Y-%m-%d"
        )
        filtered_date = (
...
def graph_data():
    ...
    username = current_user.username
    total_dates = (
        db.session.execute(select(Tasks.date).filter_by(username=username).distinct())
        .scalars()
        .all()
    )

    for datee in total_dates:
        assigned_task = db.session.execute(
            select(func.count(Tasks.taskName)).where(
                Tasks.username == current_user.username, Tasks.date == datee
            )
        ).scalar()
...
def tasks():
    ...
    taskList = (
        db.session.execute(
            select(Tasks.taskName).filter_by(
    ...
    areCompleted = (
        db.session.execute(
            select(Tasks.completed).filter_by(
                username=current_user.username, date=today
            )
        )
        .scalars()
    ...
    task_pending = db.session.execute(
        select(func.count(Tasks.taskName)).where(
            Tasks.username == current_user.username, Tasks.date == today
        )
    ).scalar()
    task_completed = db.session.execute(
        select(func.count(Tasks.taskName)).where(
            Tasks.username == current_user.username,
            Tasks.date == today,
            Tasks.completed == 1,
...
def task_marking():
    if (
        not request.json
        or not request.json.get("taskId")
        or request.json.get("stat") is None
    ):
    ...
    task = db.session.execute(select(Tasks).filter_by(tId=taskId)).scalar_one_or_none()

    if task and datetime.now(tz=timezone.utc).date() == task.date:
...
def task_completion_update():
    ...
    if date_filterr == "":
        date_filterr = datetime.now(tz=timezone.utc).date()

    task_pending = db.session.execute(
        select(func.count(Tasks.taskName)).where(
            Tasks.username == current_user.username, Tasks.date == date_filterr
        )
    ).scalar()
    task_completed = db.session.execute(
        select(func.count(Tasks.taskName)).where(
            Tasks.username == current_user.username,
            Tasks.date == date_filterr,
            Tasks.completed == 1,
        )
    ).scalar()
...
def create_task():
    ...
    newTask = Tasks(
        username=username, taskName=taskName, taskNotes="", date=current_date
    )
    ...
    return render_template(
        "_task_list_update.html", taskInfo=task_ref, current_date=display_date
    ), 200  # both share same content so no need to be sceptic
...
def update_note():
    ...
    task = db.session.execute(select(Tasks).filter_by(tId=tId)).scalar_one_or_none()
...
def get_date():
    ...
    display_date = date.fromisoformat(filter_date).strftime(
        "%B %d, %Y"
    )
    taskList = (
        db.session.execute(
            select(Tasks.taskName).filter_by(
                username=current_user.username, date=filter_date
            )
        )
        .scalars()
        .all()
...
def delete_task():
    ...
    filter_date = (
        str(datetime.now(tz=timezone.utc).date())
        if request.json.get("date") == ""
        else request.json.get("date")
    )
...
def journal():
    jlists = (
        db.session.execute(
            select(Journal_entry.fileName).filter_by(username=current_user.username)
        )
        .scalars()
        .all()
    )
...
def save_journal():
    ...
    user_files = (
        db.session.execute(select(Journal_entry.fileName).filter_by(username=username))
        .scalars()
        .all()
    )
